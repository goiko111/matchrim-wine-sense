...
import { RestaurantOCRImport } from "./RestaurantOCRImport";
import { RestaurantCSVImport } from "./RestaurantCSVImport";
import { RestaurantPDFImport } from "./RestaurantPDFImport";
import { RestaurantTextImport } from "./RestaurantTextImport";
...
export const RestaurantImport = () => {
  ...
  const handleAnalyzePrices = async () => {
    ...
    setAnalyzing(true);
    try {
      const { data, error } = await supabase.functions.invoke('analyze-restaurant-prices', {
        body: { wines }
      });
  ...
  const getPriceLabel = (estado?: string) => {
    ...
    if (estado === 'bajo') return "Precio Competitivo";
        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4">
            {wines.map((wine, index) => {
              const isCompetitive = wine.analisis?.estado === 'bajo';
              return (
              <Card 
                key={index} 

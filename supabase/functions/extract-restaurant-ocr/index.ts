...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Analiza esta imagen de una carta de vinos de restaurante y extrae TODOS los vinos con sus precios de venta al público.
...
- moneda: Código de moneda (MXN, EUR, USD) - detecta del símbolo o texto ($=MXN por defecto, €=EUR)
...
Responde SOLO con un JSON válido en este formato:
{
  "wines": [
    ...
    if (!content.endsWith('}') && !content.endsWith(']')) {
      ...
      // Encontrar el último array de wines completo
      const winesMatch = content.match(/"wines"\s*:\s*\[/);
      if (winesMatch) {
        // Contar objetos completos en el array
        let depth = 0;
    ...
    console.log(`Extracted ${result.wines?.length || 0} wines with restaurant prices from image`);

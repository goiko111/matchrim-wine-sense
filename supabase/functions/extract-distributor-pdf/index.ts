...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `Analiza este PDF de una carta de distribuidor de vinos y extrae TODOS los vinos con sus precios.
...
Responde SOLO con un JSON válido en este formato:
{
  "wines": [
    ...
    console.log(`Extracted ${result.wines?.length || 0} wines with prices from PDF`);
    ...
    console.error('Error in extract-distributor-pdf:', error);
    return new Response(
